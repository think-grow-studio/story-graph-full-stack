...
function snapshotFixture(board: Board): BoardSnapshot {
  return { board, nodes: [], edges: [], boardNodes: [], boardEdges: [] };
}
...
describe("Board use-cases", () => {
  ...
  it("creates a Board only after resolving Story ownership and graph:update", async () => {
    ...
    expect(result).toMatchObject({ storyId: "story-1", name: "Main" });
