...
Selected Nodes use a Graph Indigo border plus a structural ring so selection is not encoded by color alone. During a connection gesture, potential target Nodes use the graph-target soft tint and visible ports. Selecting a Node emphasizes its connected subgraph and dims unrelated Nodes/Relationships. Selecting a Relationship pair emphasizes its shared rail and endpoints while unrelated rails dim. Selected rails use stronger stroke width in addition to color. Clicking the empty canvas or pressing Escape when no modal workflow owns Escape clears graph focus.
...
Multiple semantic Relationships between the same unordered Node pair remain independent records, but they render as exactly one shared visual rail. Direction is summarized at the Node/rail endpoints: a directed Relationship toward either Node adds an arrow marker at that endpoint, so opposite directed meanings naturally produce arrows at both ends. The rail midpoint permanently shows a compact directional summary such as `→ 좋아한다 · ← 잊었다`; undirected meanings use `—`, long sets are truncated, and additional meanings collapse to `+N`. Users should be able to understand the essential relationship without first hovering.

Clicking or tapping the shared rail selects the Node pair as one editing unit and opens the pair-level Relationship Inspector. The Inspector presents both canonical directions (`A → B` and `B → A`) with independent activation controls. An active direction exposes its Relationship text and description. Enabling a missing direction creates one directed semantic Relationship through the standard command/save pipeline; disabling a direction removes that direction through the same history/save pipeline. Autosave, Undo/Redo, optimistic persistence, retry, and CAS behavior are reused rather than reimplemented for the pair UI.

Hovering or keyboard-focusing the rail may reveal a compact relationship preview card at the midpoint. The preview states each underlying semantic direction and Relationship name and may still select an individual legacy Relationship, but it is secondary discovery UI rather than the primary editing model. Editing belongs in the pair Inspector. The card and midpoint summary use existing Surface, Line, Ink, Muted, Graph Indigo, radius, and focus tokens and stay visually secondary to the graph.

A shared rail uses one deterministic representative Relationship for its persisted route and stroke presentation. The pair Inspector keeps advanced routing and semantic property controls tucked under `고급 설정` for the active representative Relationship. This is a visual projection rule only: every Relationship keeps its own semantic data, routing, presentation, autosave, history, and conflict behavior. Legacy duplicate Relationships in one direction remain valid records; the compact pair UI may expose a representative editor and an explicit duplicate-count note rather than duplicating rails.

### Semantic data versus presentation

Node/Relationship `properties` are semantic user data with string scalar leaves and structured object/array nesting. Presentation (`NodePresentation`, `EdgePresentation`) and Relationship routing (`EdgeRouting`) are persisted separately from semantic properties. Rail hover/preview state, focus/dimming, port hover, selected pair state, and target-pick state are transient frontend projections and must never enter persisted graph data. The Inspector edits properties through structured key/value controls; raw JSON editing is not part of the product UI.
